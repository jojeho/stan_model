...
transformed parameters {
  ...
  { // Calculate log p(state at t = j | history up to t) recursively
    ...
    log_alpha[1, 1] = log(0.8) + normal_lpdf(y[1] | 0, sigma_t[1, 1]);
    log_alpha[1, 2] = log(0.2) + normal_lpdf(y[1] | 0, sigma_t[1, 2]);
...
model {
  ...
  //p_remain ~ beta(2, 2); // Weakly informative way to say that states are sort-of sticky
